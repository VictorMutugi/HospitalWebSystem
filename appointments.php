    ...
    if ($conn->query($sql) === TRUE) {
        ...
        if ($result !== false && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr><td>" . $row["AIndex"] . "</td><td>" . $row["Full_Name"] . "</td><td>" . $row["Email"] . "</td><td>" . $row["Date"] . "</td><td>" . $row["Time"] . "</td><td>" . $row["Medic"] . "</td><td>" . $row["Issue"] . "</td></tr>";
            }
            
            echo "</table>";

            echo "<a href=\"appTable\" download>Download Receipt</a>";
        } else {
            echo "No data found";
        }
    } else {
